...
export default function ProductDetailsClient({ initialProduct, productId }: { initialProduct: Product | null, productId: string }) {
    ...
    const [features, setFeatures] = useState({ productReviews: true, relatedProducts: true });

    useEffect(() => {
        // Fetch specific product page config features
        axios.get(`${process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000"}/api/settings`)
            .then(res => { if (res.data.features) setFeatures(res.data.features); }).catch(() => { });
