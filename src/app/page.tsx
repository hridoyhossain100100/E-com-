...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    axios.get("/api/settings")
      .then(r => { if (r.data.banner) setBanner(r.data.banner); }).catch(() => { });
  ...
  return (
    <>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
        ...
        {/* ═══ ANIMATED HERO ═══ */}
        <div className="hero-gradient rounded-3xl p-8 sm:p-12 lg:p-16 mb-12 sm:mb-16 relative overflow-hidden">
          {/* Floating shapes */}
          ...
          <div className="text-center relative z-10">
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/5 border border-white/10 mb-6 backdrop-blur-sm">
              ...
              <span className="text-sm text-violet-300">Premium Collection</span>
            </div>
            <h1 className="text-4xl sm:text-5xl lg:text-7xl font-bold mb-4 tracking-tight">
              <span className="gradient-text">Discover Quality</span>
              <br /><span className="text-white">Products</span>
            </h1>
            <p className="text-gray-400 text-base sm:text-lg max-w-xl mx-auto mb-8">
              Curated collection of premium products. Shop with confidence, pay with Bkash, Nagad or Rocket.
            </p>
            {/* Search Bar */}
      ...
      <footer className="border-t border-gray-800/50 mt-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 lg:gap-12">
            ...
            <div className="sm:col-span-2 lg:col-span-1">
              ...
              <p className="text-sm text-[var(--text-muted)] leading-relaxed mb-4">
                Your trusted destination for premium products in Bangladesh. Quality guaranteed.
              </p>
              <div className="flex gap-3">
                <a href="#" className="w-9 h-9 rounded-lg bg-[var(--card-bg)] border border-[var(--card-border)] flex items-center justify-center text-[var(--text-muted)] hover:text-blue-500 hover:border-blue-500/30 transition-all"><Facebook className="w-4 h-4" /></a>
                <a href="#" className="w-9 h-9 rounded-lg bg-[var(--card-bg)] border border-[var(--card-border)] flex items-center justify-center text-[var(--text-muted)] hover:text-pink-500 hover:border-pink-500/30 transition-all"><Instagram className="w-4 h-4" /></a>
                <a href="#" className="w-9 h-9 rounded-lg bg-[var(--card-bg)] border border-[var(--card-border)] flex items-center justify-center text-[var(--text-muted)] hover:text-emerald-500 hover:border-emerald-500/30 transition-all">
            ...
            <div>
              ...
              <ul className="space-y-2.5">
                {[{ label: "Shop", href: "/" }, { label: "Checkout", href: "/checkout" }, { label: "Wishlist", href: "/wishlist" }].map(l => (
            ...
            <div>
              ...
              <ul className="space-y-2.5">
                <li className="flex gap-3 text-sm text-[var(--text-muted)]">
                  ...
                  <span>123 Commerce Avenue<br />Dhaka, Bangladesh</span>
                </li>
                <li className="flex items-center gap-2 text-sm text-[var(--text-muted)]"><Phone className="w-4 h-4 text-gray-600" />+880 1XXXXXXXXX</li>
                <li className="flex items-center gap-2 text-sm text-[var(--text-muted)]"><Mail className="w-4 h-4 text-gray-600" />support@shopvibe.com</li>
              </ul>
            </div>

            {/* Payment */}
            <div>
              <h4 className="font-semibold text-sm text-white mb-4 uppercase tracking-wider">We Accept</h4>
          ...
          <div className="border-t border-gray-800/50 mt-10 pt-8 text-center">
            <p className="text-xs text-gray-600">© {new Date().getFullYear()} ShopVibe. All rights reserved. Made with 💜 in Bangladesh</p>
          </div>
        </div>
      </footer>
      ...
      {showScrollTop && (
        <button onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })}
          className="fixed bottom-6 right-6 z-50 w-12 h-12 rounded-full bg-gradient-to-r from-violet-600 to-fuchsia-600 text-white shadow-lg shadow-violet-500/30 flex items-center justify-center scroll-top-btn hover:shadow-violet-500/50 transition-shadow">
